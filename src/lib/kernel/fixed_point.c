...
static const int CONVERSION_FACTOR = 1 << TAIL_DIGITS;
...
int to_integer_rounded(fixed_point fixed_point)
{
	if(x >= 0)
    {
		  return (fixed_point.value + (CONVERSION_FACTOR/2));
	  }
  else
    {
      return (fixed_point.value - (CONVERSION_FACTOR/2));
...
{
  fixed_point new_fixed_point;
  new_fixed_point.value = ((int_64_t) x) * CONVERSION_FACTOR / y.value;
  return new_fixed_point
}
